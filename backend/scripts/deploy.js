...
const { KOKARTS_NFT_CONTRACT_ADDRESS } = require("../constants");
...
async function main() {
  ...
  const KokArtsDAO = await ethers.getContractFactory("KokArtsDAO");
  const kokArtsDAO = await KokArtsDAO.deploy(
  ...
  await kokArtsDAO.deployed();

  console.log("KokArtsDAO deployed to: ", kokArtsDAO.address);
}
